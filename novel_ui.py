...
with st.sidebar:
    ...
    with tab_help:
        st.markdown("""
            ...
            ### 🚀 빠른 시작 가이드 (초보자용)
            1. **사이드바 열기**: 좌측의 [설정] 탭을 클릭하세요. (모바일은 좌측 상단 `>` 아이콘 클릭)
            2. **API 키 발급**: **[Get API Key]** 버튼을 눌러 Google AI Studio에 접속합니다.
            3. **키 생성**: Google 계정 로그인 후 **[Create API key]** 버튼을 클릭하여 키를 만듭니다. (이름은 자유)
            4. **키 복사**: 생성된 키 목록 우측의 복사(종이 모양) 아이콘을 클릭합니다.
            5. **키 등록**: 본 앱의 [설정] 탭 API 키 입력란에 붙여넣고 **[키 적용]** 버튼을 누르세요.
            6. **설정 완료**: 메인화면의 [기획 및 설정] 탭에서 소설 기본 설정(메인 플롯, 캐릭터 등)을 작성합니다.
            7. **집필 시작**: 메인화면의 [소설 집필실] 탭의 하단의 대화창에 원하는 내용을 입력하면 제미나이가 소설을 이어 써줍니다.
            
            ---

            ### 💾 데이터 보관 및 보안 주의사항
            * **휘발성 주의**: 브라우저를 닫으면 정보가 초기화됩니다. **[ℹ️ 정보]** 탭에서 **'현재 이야기 & 설정 저장'**을 수시로 눌러 `.json` 파일을 소장하세요.
            * **API 키 공유 금지**: 다운로드한 `.json` 파일에는 작가님의 **개인 API 키**가 포함되어 있습니다.
                - **파일을 타인과 공유할 경우**: 메모장 등으로 파일을 열어 `"api_key": "..."` 부분의 따옴표 안 내용을 삭제한 뒤 공유하세요.
            * **텍스트 추출**: 소설 본문만 따로 받고 싶다면 **[🛠️ 도구]** 탭의 **'현재 이야기 다운로드'**를 이용하세요.

            ---

            ### 🔑 개인 API 키 사용 안내 (Gemini 기준)
            * **무료 티어 이용 제한**:
                - **RPM**: 분당 최대 **15회** (초과 시 잠시 차단)
                - **RPD**: 일일 최대 **1,500회**
                - **TPM**: 분당 최대 **100만 토큰** (본문이 매우 길어질 경우 영향)
    
            > **Tip**: 답변이 끊기거나 에러가 난다면 약 1분 뒤에 다시 시도해 주세요.
        """)
        ...
        with col_key1:
            if st.button("✅ 적용", use_container_width=True):
                if user_key_input:
                    ...
                    st.rerun()
        with col_key2:
            st.link_button("🔑 Get API Key", "https://aistudio.google.com/app/apikey", use_container_width=True)
